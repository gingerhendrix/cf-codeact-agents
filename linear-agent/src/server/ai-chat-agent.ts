...
> {
  ...
  override async onMessage(connection: Connection, message: WSMessage) {
    if (typeof message === "string") {
      ...
      if (data.type === MessageType.CF_AGENT_CHAT_CLEAR) {
        ...
        await this.persistMessages(this.messages, [connection.id]);
  ...
  private async _reply(id: string, response: Response) {
    return this._tryCatchChat(async () => {
          ...
          if (isSSE) {
            ...
            for (const line of lines) {
              if (line.startsWith("data: ") && line !== "data: [DONE]") {
                try {
                  const data: UIMessageChunk<unknown, { result : { message: string } }> = JSON.parse(line.slice(6)); // Remove 'data: ' prefix
                  switch (data.type) {
                    case "text-start": {
                    ...
                    case "data-result": {
                      const resultPart = { type: "text", text: data.data.message } as TextUIPart;
                      messages.push({
      ...
      await this.persistMessages([...this.messages, ...messages.filter(m => m.parts.length > 0)]);
    });
  }
